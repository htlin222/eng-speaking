---
title: "Day 07 — Work & Career"
---

# Day 07 — Work & Career

## Sentences

- It's one of those weeks where everything lands at once.
- I'm trying to get ahead of it before it gets out of hand.
- I've been doing a lot of thinking about where this is heading.
- The work itself is fine — it's the environment that wears you down.
- I'd rather do fewer things well than more things poorly.
- I need something that gives me a reason to show up.
- There's a difference between being busy and being productive.
- I'm at a point where I'm asking different questions.
- It pays well, but that's no longer enough.
- I want to feel like what I do matters, at least a little.

## Dialog

> A: How's work been treating you lately?
> B: Honestly, I'm hitting a wall. Nothing's wrong exactly, but nothing feels right either.
> A: That might be worse than something being obviously wrong.
> B: Yeah. At least a clear problem has a solution.
> A: Have you thought about what you'd actually want instead?
> B: That's the question I keep avoiding.

## Note

**"Hitting a wall"** = reaching a point where progress stalls, often emotionally rather than technically. More nuanced than "I'm stuck."

---

## Practice

### Blind Practice

Cover the Sentences and Dialog above. Without looking, say the key sentences aloud from memory. Then open the page and see what you missed.

→ [[guide/blind-practice|How to do Blind Practice]]

### Recording Task

Record yourself going through the Dialog once, without stopping to correct yourself.

**Today's focus:** Weight. These sentences carry frustration or realisation. Let that in.

Listen back once. Don't over-analyse.

### Self-Check

- [ ] I did the Blind Practice before recording
- [ ] I recorded without stopping mid-sentence
- [ ] I listened back to my recording

→ [[guide/self-assessment|Score your recording]]
